...
|---|---|
| `productId` / `product_id` | `get_product` response — top-level `id` field. This is the UUID for ALL calls: `get_availability`, `list_product_addons`, `list_addon_groups_for_product`, `list_product_media`, `list_product_information`, `add_to_cart`. Before any of these calls, locate this value in the `get_product` tool response in the current thread. |
