...
**On input**: if the conversation context contains a `<<<memory>>>` block, load its JSON values as the current booking state before processing anything else. These values override anything inferred from conversation text.
...
| Field | Valid sources |
|---|---|
| `productId` / `product_id` | KnowledgeBase (`**ID:**` field) **or** `list_products_brief` / `get_product` response |
| `product_variant_id` | `get_product` response (`variants[].id`) only |
| `time_slot_id` | `get_product` response (`time_slots[].id`) only |
| `availability_slot_id` | `get_availability` response only |
| `addon_id` | `list_product_addons` response only |
| `cart_id` | `add_to_cart` response only |

**NEVER** for runtime IDs (`product_variant_id`, `time_slot_id`, `availability_slot_id`, `addon_id`, `cart_id`):
