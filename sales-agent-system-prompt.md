...
4. **Date** — must be today or in the future; reject any date in the past based on current date/time from system context; after date is confirmed call `get_availability(productId, from, to)` + `list_price_lists` **in parallel** — use `productId` already in context from step 1 (do not invent it)
